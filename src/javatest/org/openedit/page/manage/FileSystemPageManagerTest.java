...
public class FileSystemPageManagerTest extends BaseTestCase
{
	...
	/**
	 * writeConfig() method was tested but never used! It seems like we could have
	 * getPageConfiguration() and putPageConfiguration() methods on the PageManager instead of
	 * getPageConfigurationReader() and getPageConfigurationWriter() methods.
	 * 
	 ...
	 * assertTrue(!getConfigFile(PATH_WITHOUT_CONFIG).exists()); }
	 * 
	 * public void testWriteConfig_Exists() throws Exception { File configFile =
	 * getConfigFile(PATH_WITH_CONFIG); String oldContents = getFileContents(configFile);
	 * 
	 * try { fieldPageManager.writeConfig(PATH_WITH_CONFIG, NEW_CONFIG);
	 * assertEquals(getFileContents(configFile), NEW_CONFIG); } finally { FileWriter writer = new
	 * FileWriter(configFile); writer.write(oldContents); writer.close(); } }
	 * 
	 * public void testWriteConfig_NoConfig() throws Exception { File configFile =
	 * getConfigFile(PATH_WITHOUT_CONFIG);
	 * 
	 * try { fieldPageManager.writeConfig(PATH_WITHOUT_CONFIG, NEW_CONFIG);
	 * assertEquals(getFileContents(configFile), NEW_CONFIG); } finally { configFile.delete(); } }
	 * 
	 * public void testWriteConfig_NoSuchPath() throws Exception { File configFile =
	 * getConfigFile(NONEXISTENT_PATH);
	 * 
	 * try { fieldPageManager.writeConfig(NONEXISTENT_PATH, NEW_CONFIG);
	 * assertEquals(getFileContents(configFile), NEW_CONFIG); } finally { configFile.delete(); } }
	 */
