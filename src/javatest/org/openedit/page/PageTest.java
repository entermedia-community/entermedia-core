...
public class PageTest extends BaseTestCase
{
	...
	public void testSimpleXconf() throws Exception
	{
		...
		// getFixture().getPageManager().getPageSettingsManager().getPageSettings("/simple/test.html");
		//
		// assertEquals("/WEB-INF/base/simple/test.html",settings.getXConf().getActualPath());
	}

	/*
	 * public void testFallBackRedirected() throws Exception { //a falls back to a_fallback //b falls
	 * back to b_fallback
	 * 
	 * Page page =
	 * getFixture().getPageManager().getPage("/fallbackredirect_modules/asset/editor/test.html"); String
	 * path = page.getPageSettings().getFallback().getPath();
	...
	/*
	 * public void testShadowFallBack2() throws Exception { Page page = getPage(
	 * "/appwithshadow2/AToB/inb.html" );
	 * 
	 ...
	 * assertNotNull("Property should exist",appid); assertEquals("appwithshadow2",appid.getValue());
	 * 
	 ...
	 * assertEquals("/appwithshadow2/AToB/_site.xconf",AToB.getParent().getPath());
	 * 
	 * PageSettings fallback = getFixture().getPageManager().getPageSettingsManager().getPageSettings(
	 * "/WEB-INF/base/ShadowFallBack2/AToB/_site.xconf"); PageProperty baseprop =
	 * fallback.getProperty("fallbackdirectory"); assertNotNull("Property should exist",baseprop);
	 * assertEquals("/WEB-INF/base/ShadowFallBack2/AToB/_site.xconf",baseprop.getPath());
	 * 
	 * 
	 * // assertEquals("/WEB-INF/base/ShadowFallBack/AToB/_site.xconf",AToB.load); // // PageSettings B
	 * = AToB.getParent().getFallback().getFallback(); //
	...
	protected void checkFilter(Filter inFilter)
	{
		...
		assertTrue("First AndFilter child should be a UserFilter", andFilter.getFilters()[0] instanceof UserFilter);
		...
		assertTrue("Second AndFilter child should be an OrFilter", andFilter.getFilters()[1] instanceof OrFilter);

		OrFilter orFilter = (OrFilter) andFilter.getFilters()[1];
		assertEquals("Number of OrFilter children", 2, orFilter.getFilters().length);

		assertTrue("First OrFilter child should be a GroupFilter", orFilter.getFilters()[0] instanceof GroupFilter);

		GroupFilter groupFilter = (GroupFilter) orFilter.getFilters()[0];
		assertEquals("foo", groupFilter.getGroupId());

		assertTrue("Second OrFilter child should be a PermissionFilter", orFilter.getFilters()[1] instanceof PermissionFilter);

		PermissionFilter permissionFilter = (PermissionFilter) orFilter.getFilters()[1];
		assertEquals("bar", permissionFilter.getPermission());

		assertTrue("Third AndFilter child should be a NotFilter", andFilter.getFilters()[2] instanceof NotFilter);

		NotFilter notFilter = (NotFilter) andFilter.getFilters()[2];
		assertTrue("NotFilter's child should be a UserFilter", notFilter.getFilter() instanceof UserFilter);
