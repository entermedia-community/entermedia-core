...
public class EventManager {
	...
	public Collection getFilteredListeners(String inCatalogId) {
		...
		if (list == null) {
			...
			for (Iterator iterator = listeners.iterator(); iterator.hasNext();) {
				...
				WebEventListener listener = (WebEventListener) getSearcherManager().getModuleManager()
						.getBean(inCatalogId, bean);

				Collection actionlist = (Collection) getSearcherManager().query(inCatalogId, "webeventlistenerfilter")
						.match("webeventlistener", data.getId()).search();
	 ...
	 * if (viewid != null)
	 * {
	 * Data view = getSearcherManager().getCachedData(inSearcher.getCatalogId(),
	 * "view", viewid);
	 * changesstring = view.getName() + " \n" + changesstring;
	 * }
	 * event.setProperty("details", changesstring);
		...
		for (Iterator iterator = fields.iterator(); iterator.hasNext();) {
			...
			if (detail != null && detail.isList()) {
				if (newvalue instanceof String && oldval instanceof String) {
					Data data = (Data) getSearcherManager().getCachedData(detail.getListCatalogId(), detail.getListId(),
							(String) oldval);
					if (data != null) {
						oldval = data.getName();
					}
					Data newdata = (Data) getSearcherManager().getCachedData(detail.getListCatalogId(),
							detail.getListId(), (String) newvalue);
					if (newdata != null) {
