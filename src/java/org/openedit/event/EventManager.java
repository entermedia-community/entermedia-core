...
public class EventManager
{
	 ...
	 * 
	 * if (changes.length() > 0) { inReq.putPageValue("datachanges", changes);
	 * 
	 * WebEvent event = new WebEvent(); event.setCatalogId(inSearcher.getCatalogId()); String type =
	 * inSearcher.getSearchType(); event.setSearchType(type); event.setSource(this);
	 * event.setOperation("edit"); event.setSourcePath(object.getSourcePath()); event.setProperty("id",
	 * object.getId()); event.setProperty(inSearcher.getSearchType() + "id", object.getId());
	 * 
	 * event.setProperty("sourcepath", object.getSourcePath());
	 * 
	 * //aka "changes" String changesstring = changes.toString();
	 * 
	 * String viewid = inReq.findValue("viewid"); event.setProperty("viewid", viewid);
	 * 
	 * if (viewid != null) { Data view = getSearcherManager().getCachedData(inSearcher.getCatalogId(),
	 * "view", viewid); changesstring = view.getName() + " \n" + changesstring; }
	 * event.setProperty("details", changesstring);
	 * 
	 * if (inSearcher.getSearchType().equals("asseteditLog")) { String jsonsource = readJSON(inReq,
	 * inSearcher, object); event.setValue("detailsjson", jsonsource); } event.setUser(inReq.getUser());
	 * fireEvent(event); } }
	 */
