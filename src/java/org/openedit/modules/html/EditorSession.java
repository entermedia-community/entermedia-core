...
public class EditorSession extends EditSession
{
	...
	/*
	 * public HtmlWysiwygConverter getWysiwygConverter( WebPageRequest inContext ) { if
	 * (fieldWysiwygConverter == null) { String userAgent = inContext.getRequest().getHeader(
	 * "User-Agent" ); if ( userAgent.indexOf("Gecko") > -1 ) { fieldWysiwygConverter = new
	 * MozillaHtmlWysiwygConverter(); } else { if ( userAgent.indexOf( "MSIE 6.0") > -1 ) {
	 * fieldWysiwygConverter = new IE60HtmlWysiwygConverter(); } else { fieldWysiwygConverter = new
	 * IE55HtmlWysiwygConverter(); } } } return fieldWysiwygConverter; }
	 */
	/*
	 * public String escapeSource(String inContent, WebPageRequest inContext) throws Exception { String
	 * sourceContent = getSourceConverter().toDisplayCode(inContent); String finalHtml =
	 * "<html><head><base href='" + getBasePath() + "'>"; finalHtml += "<style type='text/css'>";
	 * finalHtml += getExternalCss(); finalHtml += "</style>"; finalHtml += "</head>"; finalHtml +=
	 * BODYSTART; finalHtml += URLUtilities.xmlEscape( sourceContent); finalHtml += BODYEND; finalHtml
	 * += "</html>";
	 * 
	 * return finalHtml; }
	 */
	...
	/*
	 * public HtmlSourceConverter getSourceConverter() { if (fieldSourceConverter == null) {
	 * fieldSourceConverter = new HtmlSourceConverter(); } return fieldSourceConverter; }
	 */
