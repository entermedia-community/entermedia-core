...
public class XmlFolderSearcher extends XmlSearcher {
	...
	protected XmlFile loadXmlFile() {
		try {
			...
			long inLastModified = getPageManager().getRepository().getStub(composite.getPath()).lastModified()
					.getTime();
			...
			List<String> children2 = getPageManager()
					.getChildrenPaths("/" + getCatalogId() + "/data" + "/lists/" + inName + "/", true);
			if (children2.size() > 0) {
				composite.setExist(true);
			...
			//
			// List<String> children3 = getPageManager().getChildrenPaths("/" + catalogid +
			// "/data" + "/lists/"
			// + inName + "/",true);
			// if( children3.size() > 0)
			// {
