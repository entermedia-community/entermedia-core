...
public class PageSettingsToXconfWriter
{
	...
	protected void saveProperties(PageSettings inProperties, XconfConfiguration inConfig)
	{
		...
		// {
		// inConfig.saveProperty("encoding",inProperties.getFieldPageCharacterEncoding(),
		// null);
		// }
	}
	...
	/**
	 * Handle this one day inConfig.removeElements("generator");
	 * 
	 * 
	 * //the choices are velocity-xslt xslt velocity jsp if ( inPageSetting.getFieldGenerator() != null)
	 * { inPageSetting.getFieldGenerator(); for (Iterator iter = .iterator(); iter.hasNext();) {
	 * Generator gen = (Generator) iter.next(); //TODO: if ( gen instanceof NestedGenerator) handle
	 * nested and mimetypes
	 * 
	 * String generator = gen.getName();
	 * 
	 * XMLConfiguration currentGen = (XMLConfiguration) inConfig.addChild("generator");
	 * currentGen.setAttribute("name", generator); //TODO: currentGen.setAttribute("mimetypes",
	 * generator.); } }
	 */
	// protected void saveContentFile(PageSettings inPageSetting, XconfConfiguration
	// inConfig)
	// {
	// //content file
