...
public class PageSettingsToXconfWriter {
	...
	public ContentItem createXConf(PageSettings inSetting) throws OpenEditException {
		...
		if (inSetting.getXConf().exists()) {
			config.populate(
					fieldXmlUtil.getXml(inSetting.getXConf().getInputStream(), inSetting.getPageCharacterEncoding()));
		}
		...
		ReaderItem results = new ReaderItem(inSetting.getPath(), new StringReader(out.toString()),
				inSetting.getPageCharacterEncoding());
		return results;
	}
	 ...
	 * 
	 * XMLConfiguration currentGen = (XMLConfiguration)
	 * inConfig.addChild("generator");
