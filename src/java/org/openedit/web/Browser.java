...
public class Browser
{
	...
	/**
	 * FIXME: This is a temporary method needed because Velocity can't handle static fields (it would
	 * seem). This should disappear when we fix bug WSP35.
	 *
	 * @return
	 */
	public boolean isXMLEditorCompatible()
	{
		return (getBrowserType() == GECKO_BROWSER) || ((getBrowserType() == MSIE_BROWSER) && (getVersion().compareTo("5.5") >= 0));
	}
	...
	protected void parseUserAgent()
	{
		...
		boolean isMobile = Pattern.compile("Android|iPhone|iPad|iPod", Pattern.CASE_INSENSITIVE).matcher(getUserAgent()).find();
	...
	private int findBrowser(String browVer)
	{
		...
		if (browVer.startsWith("Safari") || browVer.contains("AppleWebKit"))
		{
			...
			if (browVer.startsWith("Gecko"))
			{
				return GECKO_BROWSER;
			}
			else
				if (browVer.startsWith("Lynx"))
				{
					return TEXT_BROWSER;
				}
				else
					if (browVer.startsWith("MSIE"))
					{
						return MSIE_BROWSER;
					}
					else
						if (browVer.startsWith("Opera"))
						{
							return OPERA_BROWSER;
						}
						else
							if (browVer.startsWith("Mozilla"))
							{
