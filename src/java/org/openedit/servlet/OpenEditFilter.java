...
public class OpenEditFilter implements Filter
{
	...
	public void destroy()
	{
		// We try to shutdown in three ways 1. Filter.destroy() 2. Web.xml
		// ShutdownListener.java 3. BaseWebServer.initialize() shutdown hook
		getEngine().shutdown();
	}
	...
	public void doFilter(ServletRequest inRequest, ServletResponse inResponse, FilterChain chain) throws IOException, ServletException
	{
		...
		{
			request.getSession(true).setAttribute("moduleManager", getEngine().getModuleManager()); // This is needed so
																									// we can save the
																									// moduleManager
		...
		if (getEngine() == null)
		{
			response.getWriter().print("<html>Open Edit Server is not initialized, please check the logs for errors</html>");

			return;
		}
		request.setAttribute("servletchain", chain); // This is used by the ServletChainGenerator to call doFilter at
														// the right time
