...
public abstract class BaseGenerator implements Generator, Cloneable
{
	...
	// protected void checkCors(HttpServletRequest httpReq, HttpServletResponse
	// httpResp, User inUser)
	// {
	...
	// else
	// {
	// httpResp.setHeader("Access-Control-Allow-Origin","*"); //This does nothing
	// useful
	// }
	// }
	//
	...
	// //{
	// //httpResp.setHeader("Access-Control-Allow-Origin", origin); //TODO: This is
	// not secure at all. my JSESSION id is avilable to any site
	// //}
	// httpResp.setHeader("Access-Control-Allow-Methods", VALID_METHODS);
	//
	...
	protected boolean ignoreError(Throwable inWrapped)
	{
		...
		if (message.indexOf("Broken pipe") > -1 || message.indexOf("socket write error") > -1 || message.indexOf("Connection reset") > -1) // tomcat
		{
			return true;
		}
	...
	protected void setHeaders(HttpServletResponse res, Long lastmodified)
	{
		...
		// res.setDateHeader("Expires", now + (1000 * 60 * 60 * 24 * 30 * 6 )); //sec *
		// min * hour * 48 Hours six months
	}
