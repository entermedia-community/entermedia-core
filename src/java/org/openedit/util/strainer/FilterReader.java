...
public class FilterReader
{
	 ...
	 * 
	 * @return A filter, or <code>null</code>, depending on the configuration passed in:
	 *         <ul>
	 *         <li><code>null</code> if the configuration is null or has no children;
	 *         <li>the corresponding filter, if the configuration has exactly one child; or
	 *         <li>an <code>AndFilter</code> containing all the children, if there is more than one
	 *         child.
	 *         </ul>
	 * 
	 * @throws ConfigurationException
	 */
	public Filter readFilterCollection(Configuration inConfig, String inPermission) throws OpenEditException
	{
		Filter filter = null;
		...
		if (inConfig != null)
		{
			...
			switch (elements.size())
			{
				...
				case 1:
					filter = readFilter((Configuration) elements.get(0), inPermission);
				...
				default:
					...
					{
						filters[i] = readFilter((Configuration) elements.get(i), inPermission);
					}
	 ...
	 * 
	 * @throws ConfigurationException If the configuration could not be decoded successfully
	 */
	protected Filter readFilter(Configuration inConfig, String inPermission) throws OpenEditException
	{
		if (inConfig == null)
		{
			return null;
		...
		if (elemName.equals("and"))
		{
			...
			if (elemName.equals("or"))
			{
				result = new OrFilter(readSubFilters(inConfig, inPermission));
			}
			else
				if (elemName.equals("not"))
				{
					if (elements.size() > 1)
					{
						throw new OpenEditException("<not> element must have one or less children");
					}
					if (elements.size() == 0)
					{
					...
					{
						result = new NotFilter(readFilter((Configuration) elements.get(0), inPermission));
					}
				}
				else
					if (elemName.equals("user"))
					{
						result = new UserFilter(inConfig.getAttribute("name"));
					}
					else
						if (elemName.equals("userprofile"))
						{
							...
							if (elemName.equals("group"))
							{
								...
								if (elemName.equals("settingsgroup"))
								{
									String id = inConfig.getAttribute("id");
									if (id == null)
									{
									...
									if (elemName.equals("permission"))
									{
										...
										if (elemName.equals("path"))
										{
											result = new PathFilter(inConfig.getAttribute("name"));
										}
										else
											if (elemName.equals("page-property") || elemName.equals("pageproperty"))
											{
												result = new PagePropertyFilter(inConfig.getAttribute("name"), inConfig.getAttribute("equals"));
											}
											else
												if (elemName.equals("pagevalue") || elements.equals("context-variable"))
												{
													result = new PageValueFilter(inConfig.getAttribute("name"), inConfig.getAttribute("equals"));
												}
												// not used much
												else
													if (elemName.equals("request-attribute"))
													{
														result = new RequestAttributeFilter(inConfig.getAttribute("name"), inConfig.getAttribute("equals"));
													}
													else
														if (elemName.equals("blank"))
														{
															result = new BlankFilter();
														}
														else
															if (elemName.equals("boolean"))
															{
																...
																if (elemName.equals("action"))
																{
																	...
																	if (elemName.equals("referer"))
																	{
																		...
																		if (elemName.equals("dataproperty"))
																		{
																			...
																			if (elemName.equals("userproperty"))
																			{
																				result = new UserPropertyFilter(inConfig.getAttribute("name"), inConfig.getAttribute("value"));
																			}
																			else
																				if (elemName.equals("groupproperty"))
																				{
																					result = new GroupPropertyFilter(inConfig.getAttribute("name"), inConfig.getAttribute("value"));
																				}

		if (result == null)
		{
			result = (Filter) getModuleManager().getBean(elemName + "Filter");
		}
		if (result == null)
		{
			throw new OpenEditException("Unrecognized filter element <" + elemName + ">");
		}
	 ...
	 * 
	 * @param inConfig The configuration whose children should be parsed
	 * 
	 * @return Filter[] The decoded filters
	 * 
	 * @throws ConfigurationException If the configuration could not be decoded successfully
	 */
	protected Filter[] readSubFilters(Configuration inConfig, String inPermission) throws OpenEditException
	{
		List elements = inConfig.getChildren();
		...
		{
			subFilters[index] = readFilter((Configuration) iter.next(), inPermission);
			index++;
		}
