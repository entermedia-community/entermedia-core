...
public class LocaleManager
{
	 ...
	 * 
	 * The given delimiters string is supposed to consist of any number of delimiter characters. Each of
	 * those characters can be used to separate tokens. A delimiter is always a single character; for
	 * multi-character delimiters, consider using delimitedListToStringArray
	 * 
	 ...
	 * @param ignoreEmptyTokens omit empty tokens from the result array (only applies to tokens that are
	 *        empty after trimming; StringTokenizer will not consider subsequent delimiters as token in
	 *        the first place).
